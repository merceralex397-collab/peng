...
- [ ] [pre-review] Step 5 — Run and record formatting checks scoped only to the plan-authorized Rust source files, full Rust tests, clippy, Svelte check, Vite build, no-bundle Tauri release build, diff check, and scope audit; stop on any non-pass or undeclared change and do not merge or start another ticket.
